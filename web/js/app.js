document.addEventListener('DOMContentLoaded', async () => {
    ...
    function updateSidebar(algData) {
        ...
        document.getElementById('val-distance').textContent = `${gs.total_distance_km.toFixed(2)}`;
        document.getElementById('val-time').textContent     = `${gs.total_time_minutes.toFixed(1)}`;
    ...
    function buildCompareCards(compData, selectedAlg) {
        ...
        const runtimes = algs.map(a => compData[a].runtime).filter(r => r !== null);
